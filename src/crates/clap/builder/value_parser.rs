...
use crate::crates::clap::derive::ValueEnum;
use crate::crates::clap::parser::ValueSource;
use crate::crates::clap::util::AnyValue;
...
impl ValueParser {
    ...
    pub fn parse_ref(
        ...
        value: &std::ffi::OsStr,
        source: ValueSource,
    ) -> Result<AnyValue, crate::crates::clap::Error> {
    ...
    pub fn possible_values(
        &self,
    ) -> Option<Box<dyn Iterator<Item = crate::crates::clap::builder::PossibleValue> + '_>> {
...
/// [`PossibleValue`][crate::crates::clap::builder::PossibleValue]s.
///
/// # Examples
///
/// ```rust
...
pub trait TypedValueParser: Clone + Send + Sync + 'static {
    ...
    fn parse_ref(
        ...
        arg: Option<&crate::Arg>,
        value: &std::ffi::OsStr,
    ) -> Result<Self::Value, crate::crates::clap::Error>;
    ...
    T: Send + Sync + Clone,
{
    ...
    fn parse_ref(
        ...
        let value = ok!(value.to_str().ok_or_else(|| {
            crate::crates::clap::Error::invalid_utf8(
                cmd,
                crate::crates::clap::output::Usage::new(cmd).create_usage_with_title(&[]),
            )
        }));
        let value = ok!((self)(value).map_err(|e| {
            ...
            crate::crates::clap::Error::value_validation(arg, value.to_owned(), e.into())
                .with_cmd(cmd)
        }));
        Ok(value)
    }
...
impl TypedValueParser for PathBufValueParser {
    ...
    fn parse(
        ...
        if value.is_empty() {
            return Err(crate::crates::clap::Error::empty_value(
                cmd,
                &[],
...
/// ```
#[derive(Clone, Debug)]
pub struct EnumValueParser<E: ValueEnum + Clone + Send + Sync + 'static>(
    std::marker::PhantomData<E>,
);

impl<E: ValueEnum + Clone + Send + Sync + 'static> EnumValueParser<E> {
    /// Parse an [`ValueEnum`][ValueEnum]
    pub fn new() -> Self {
...
impl<E: ValueEnum + Clone + Send + Sync + 'static> TypedValueParser for EnumValueParser<E> {
    ...
    fn parse_ref(
        ...
        let value = ok!(value.to_str().ok_or_else(|| {
            crate::crates::clap::Error::invalid_value(
                cmd,
...
/// Verify the value is from an enumerated set of [`PossibleValue`][crate::crates::clap::builder::PossibleValue].
///
/// See also:
/// - [`EnumValueParser`] for directly supporting [`ValueEnum`][ValueEnum] types
...
impl TypedValueParser for PossibleValuesParser {
    ...
    fn parse(
        ...
        arg: Option<&crate::Arg>,
        value: std::ffi::OsString,
    ) -> Result<String, crate::crates::clap::Error> {
        let value = ok!(value.into_string().map_err(|_| {
            crate::crates::clap::Error::invalid_utf8(
                cmd,
                crate::crates::clap::output::Usage::new(cmd).create_usage_with_title(&[]),
        ...
        if self.0.iter().any(|v| v.matches(&value, ignore_case)) {
            ...
            Err(crate::crates::clap::Error::invalid_value(
                cmd,
                value,
                &possible_vals,
                arg.map(ToString::to_string)
    ...
    fn parse_ref(
        ...
        let value = ok!(value.parse::<i64>().map_err(|err| {
            let arg = arg
                .map(|a| a.to_string())
                .unwrap_or_else(|| "...".to_owned());
            crate::crates::clap::Error::value_validation(
                arg,
        ...
        if !self.bounds.contains(&value) {
            let arg = arg
                .map(|a| a.to_string())
                .unwrap_or_else(|| "...".to_owned());
            return Err(crate::crates::clap::Error::value_validation(
                arg,
                raw_value.to_string_lossy().into_owned(),
...
impl BoolValueParser {
    ...
    fn possible_values() -> impl Iterator<Item = crate::crates::clap::builder::PossibleValue> {
        ["true", "false"]
            .iter()
            .copied()
            .map(crate::crates::clap::builder::PossibleValue::new)
    }
}
...
impl TypedValueParser for BoolValueParser {
    ...
    fn parse_ref(
        ...
        let value = if value == std::ffi::OsStr::new("true") {
            ...
            return Err(crate::crates::clap::Error::invalid_value(
                cmd,
                value.to_string_lossy().into_owned(),
                &possible_vals,
                arg.map(ToString::to_string)
...
impl FalseyValueParser {
    ...
    fn possible_values() -> impl Iterator<Item = crate::crates::clap::builder::PossibleValue> {
        crate::crates::clap::util::TRUE_LITERALS
            .iter()
            .chain(crate::crates::clap::util::FALSE_LITERALS.iter())
            .copied()
            .map(|l| {
                crate::crates::clap::builder::PossibleValue::new(l)
                    .hide(l != "true" && l != "false")
            })
    }
}
...
impl TypedValueParser for FalseyValueParser {
    ...
    fn parse_ref(
        ...
        let value = if value.is_empty() {
            ...
            crate::crates::clap::util::str_to_bool(value).unwrap_or(true)
        };
        Ok(value)
    }
...
impl TypedValueParser for BoolishValueParser {
    ...
    fn parse_ref(
        ...
        let value = ok!(
            crate::crates::clap::util::str_to_bool(value).ok_or_else(|| {
                let arg = arg
                    .map(|a| a.to_string())
                    .unwrap_or_else(|| "...".to_owned());
                crate::crates::clap::Error::value_validation(
...
/// When encountered, report [ErrorKind::UnknownArgument][crate::crates::clap::error::ErrorKind::UnknownArgument]
///
    ...
    fn parse_ref_(
        ...
        match source {
            ...
            ValueSource::EnvVariable | ValueSource::CommandLine => {
                ...
                let err = crate::crates::clap::Error::unknown_argument(
                    cmd,
                    arg,
                ...
                let err = {
                    debug_assert_eq!(
                        err.get(crate::crates::clap::error::ContextKind::Suggested),
                        None,
                        "Assuming `Error::unknown_argument` doesn't apply any `Suggested` so we can without caution"
                    );
                    err.insert_context_unchecked(
                        crate::crates::clap::error::ContextKind::Suggested,
                        crate::crates::clap::error::ContextValue::StyledStrs(
...
pub mod via_prelude {
    ...
    impl<E: ValueEnum + Clone + Send + Sync + 'static> _ValueParserViaValueEnum
...
macro_rules! value_parser {
    ($name:ty) => {{
        use $crate::crates::clap::builder::via_prelude::*;
        let auto = $crate::crates::clap::builder::_AutoValueParser::<$name>::new();
        (&&&&&&auto).value_parser()
    }};
}
...
mod private {
    ...
    impl<E: ValueEnum> _ValueParserViaValueEnumSealed for &&&&&_AutoValueParser<E> {}
