...
impl<F: ErrorFormatter> Error<F> {
    ...
    pub fn set_help_flag(mut self, help_flag: Option<&'static str>) -> Self {
    ...
    #[inline(never)]
    #[cfg(feature = "error-context")]
    pub fn insert_context_unchecked(mut self, kind: ContextKind, value: ContextValue) -> Self {
    ...
    pub fn unnecessary_double_dash(cmd: &Command, arg: String, usage: Option<StyledStr>) -> Self {
        use std::fmt::Write as _;
        let styles = cmd.get_styles();
        let invalid = &styles.get_invalid();
        let valid = &styles.get_valid();
