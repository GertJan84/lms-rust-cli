...
use super::builder::OsStr;
use super::{Arg, INTERNAL_ERROR_MSG};
...
impl MKeyMap {
    ...
    pub fn push(&mut self, new_arg: Arg) {
    ...
    pub fn get<K: ?Sized>(&self, key: &K) -> Option<&Arg>
    where
        KeyType: PartialEq<K>,
    {
        self.keys
    ...
    pub fn keys(&self) -> impl Iterator<Item = &KeyType> {
    ...
    pub fn mut_args<F>(&mut self, f: F)
    where
        F: FnMut(Arg) -> Arg,
    {
    ...
    /// `Some(arg)` if removed.
    pub fn remove_by_name(&mut self, name: &str) -> Option<Arg> {
        self.args
            .iter()
