...
use crate::crates::clap::clap_lex::OsStrExt;
use crate::crates::clap::{
    builder::ArgAction,
    clap_lex,
    mkeymap::KeyType,
    parser::{SubCommand, Validator},
    util::AnyValue,
    Arg, Command, Id, INTERNAL_ERROR_MSG,
};
use crate::crates::clap::{error::Error as ClapError, output::Usage};
use crate::crates::clap::{error::Result as ClapResult, parser::ValueSource};

use super::{features::suggestions, ArgMatcher};
                ...
                let mut used: Vec<Id> = matcher
                    ...
                    .filter(|arg_id| {
                        matcher.check_explicit(
                            arg_id,
                            &crate::crates::clap::builder::ArgPredicate::IsPresent,
                        )
                    })
                    .filter(|&n| {
                        self.cmd
            ...
            ArgAction::Count => {
                let raw_vals = if raw_vals.is_empty() {
                    let existing_value = *matcher
                        .get_one::<crate::crates::clap::builder::CountType>(arg.get_id().as_str())
                        .unwrap_or(&0);
            ...
            if !matcher.contains(arg.get_id()) {
                for (id, val, default) in arg.default_vals_ifs.iter() {
                    let add = if let Some(a) = matcher.get(id) {
                        match val {
                            crate::crates::clap::builder::ArgPredicate::Equals(v) => {
                                a.raw_vals_flatten().any(|value| v == value)
                            }
                            crate::crates::clap::builder::ArgPredicate::IsPresent => true,
                        }
                    } else {
                        false
                    };
        ...
        if !arg.default_vals.is_empty() {
            ...
            // do nothing
                ...
                let arg_values: Vec<_> = arg
                    .default_vals
                    .iter()
                    .map(crate::crates::clap::builder::OsStr::to_os_string)
                    .collect();
                let trailing_idx = None;
                let _ = ok!(self.react(
                    None,
